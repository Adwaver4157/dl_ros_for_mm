...
def convert_Twist(
    data, action_noise=0.1, lower_bound=[0.0, -1.5], upper_bound=[1.5, 1.5]
):
    acs = []
    for msg in tqdm(data):
        # action
...
def transform2homogeneousM(tfobj):
    ...
    tfeul = tf.transformations.euler_from_quaternion(
        [tfobj.rotation.x, tfobj.rotation.y, tfobj.rotation.z, tfobj.rotation.w],
        axes="sxyz",
    )
...
def convert_EndEffectorPose(data):
    ...
    for msg in tqdm(data):
        # print(msg)
        end_effector_pose_list.append(
            [msg.pose[0] * 0.001, msg.pose[1] * 0.001, msg.pose[2] * 0.001]
        )
